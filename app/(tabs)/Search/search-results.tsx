...
export default function SearchResults() {
  ...
  const loadMore = async () => {
    ...
    // totalCount를 사용하여 더 이상 데이터가 남아있는지 계산 가능
    if (result.totalCount !== undefined) {
      if (offset + newItems.length >= result.totalCount) {
        setHasMore(false);
      }
    } else {
      // totalCount 정보가 없으면 기존 방식 유지
  ...
  const handleSearch = async (value: string) => {
    ...
    // 더보기 버튼 노출 여부: totalCount가 limit 초과일 때만 노출
  ...
  return (
    <View className="flex-1 mb-2">
      ...
      <FlatList
        ...
        columnWrapperStyle={{ justifyContent: "space-between", paddingHorizontal: 16, marginTop: 10 }}
        renderItem={({ item }) => (
          <GoodsThumbnail
            title={item.title}
